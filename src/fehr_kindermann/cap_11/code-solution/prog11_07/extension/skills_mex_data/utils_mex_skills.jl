...
function foc(x_in)
    ...
    v_ind = v[ij_com, ia_com, ip_com, is_com, ik_com, it_com]

    # calculate the wage rate
    wage = wn[it_com]*eff[ij_com, ik_com]*theta[ip_com]*eta[is_com]

    # calculate available resources
    available = (1.0+rn[it_com])*a[ia_com, ik_com] + beq[ij_com, ik_com, it_com] + pen[ij_com, ik_com, it_com] + v_ind
    ...
    tomorrow = max(varphi*RHS[ij_com+1, ial, ip_com, is_com, ik_com, itp] + (1.0-varphi)*RHS[ij_com+1, iar, ip_com, is_com, ik_com, itp], 0.0)
...
function valuefunc(a_plus, cons, lab, ij, ip, is, ik, it)
    ...
    valuefunc = 0.0
    if (ij < JJ)
        valuefunc = max(varphi*EV[ij+1, ial, ip, is, ik, itp] + (1.0-varphi)*EV[ij+1, iar, ip, is, ik, itp], 1e-10)^egam/egam
    end
...
#        omega(9) = 1d0/9d0
    ...
    for ij in 1:JJ
        for ip in 1:NP
            for is in 1:NS
                for ik in 1:SS
                    @. aplus[ij, :, ip, is, ik, 0] = max(a[:, ik]/2.0, a[1, ik]/2.0)
                end
            end
        end
    end
...
function solve_household(ij_in, it_in)
    ...
    for ia in 0:NA
        for ik in 1:SS
            aplus[JJ, ia, :, :, ik, it] .= 0.0
            c[JJ, ia, :, :, ik, it] .= ((1.0+rn[it])*a[ia, ik]+ beq[JJ, ik, it] .+ pen[JJ, ik, it] .+ v[JJ, ia, :, :, ik, it])/p[it]#
            l[JJ, ia, :, :, ik, it] .= 0.0
            VV[JJ, ia, :, :, ik, it] .= valuefunc(0.0, c[JJ, ia, 1, 1, ik, it],l[JJ, ia, 1, 1, ik, it], JJ, 1, 1, ik, it)
        end
    end

    # interpolate individual RHS
    for ik in 1:SS
        interpolate(JJ, ik, it)
    end
        ...
        for ik in 1:SS
            ...
            for ia in 0:NA
                ...
                if (ij >= JR && ia == 0 && kappa[it] <= 1e-10)
                    aplus[ij, ia, :, :, ik, it] = 0.0
                    c[ij, ia, :, :, ik, it] = 0.0
                    l[ij, ia, :, :, ik, it] = 0.0
                    VV[ij, ia, :, :, ik, it] = valuefunc(0.0, 0.0, 0.0, ij, 1, 1, ik, it)
                    continue
                end
                ...
                for ip in 1:ip_max
                    for is in 1:is_max
                        ...
                        if (x_root < 0.0)
                            ...
                            v_ind = v[ij, ia, ip, is, ik, it]
                            available = (1.0+rn[it])*a[ia, ik] + beq[ij, ik, it] + pen[ij, ik, it] + v_ind
                            if (ij < JR)
                        ...
                        # copy decisions
                        aplus[ij, ia, ip, is, ik, it] = x_root
                        c[ij, ia, ip, is, ik, it] = cons_com
                        l[ij, ia, ip, is, ik, it] = lab_com
                        VV[ij, ia, ip, is, ik, it] = valuefunc(x_root, cons_com, lab_com, ij, ip, is, ik, it)

                    end

                    # copy decision in retirement age
                    if (ij >= JR)
                        aplus[ij, ia, :, :, ik, it] .= aplus[ij, ia, 1, 1, ik, it]
                        c[ij, ia, :, :, ik, it] .= c[ij, ia, 1, 1, ik, it]
                        l[ij, ia, :, :, ik, it] .= l[ij, ia, 1, 1, ik, it]
                        VV[ij, ia, :, :, ik, it] .= VV[ij, ia, 1, 1, ik, it]
                    end
                end
            end
    ...
    for ia in 0:NA
        ...
        for ip in 1:NP
            for is in 1:NS
                ...
                RHS[ij, ia, ip, is, ik, it] = 0.0
                EV[ij, ia, ip, is, ik, it] = 0.0
                for is_p in 1:NS
                    chelp = max(c[ij, ia, ip, is_p, ik, it],1e-10)
                    lhelp = max(l[ij, ia, ip, is_p, ik, it],1e-10)
                    RHS[ij, ia, ip, is, ik, it] = RHS[ij, ia, ip, is, ik, it] + pi[is, is_p]*margu(chelp, lhelp, it)
                    EV[ij, ia, ip, is, ik, it]  = EV[ij, ia, ip, is, ik, it] + pi[is, is_p]*VV[ij, ia, ip, is_p, ik, it]
                end
                RHS[ij, ia, ip, is, ik, it] = max((1.0+rn[it])*beta*psi[ij,it]*RHS[ij, ia, ip, is, ik, it],1e-10)^(-gamma)
                EV[ij, ia, ip, is, ik, it] = (egam*EV[ij, ia, ip, is, ik, it])^(1.0/egam)
            end
        end
    end
            ...
            for ia in 0:NA
                for ip in 1:NP
                    for is in 1:NS
                        ...
                        ial, iar, varphi = linint_Grow(aplus[ij-1, ia, ip, is, ik, itm], a_l, a_u, a_grow, NA, ial_v, iar_v, varphi_v)
                        ...
                        for is_p in 1:NS
                            phi[ij, ial, ip, is_p, ik, it] = phi[ij, ial, ip, is_p, ik, it] + pi[is, is_p]*varphi*phi[ij-1, ia, ip, is,ik,itm]
                            phi[ij, iar, ip, is_p, ik, it] = phi[ij, iar, ip, is_p, ik, it] + pi[is, is_p]*(1.0-varphi)*phi[ij-1,ia,ip,is,ik,itm]
                        end
                    end
                end
            end
    ...
    for ij in 1:JJ
        for ik in 1:SS
            for ia in 0:NA
                for ip in 1:NP
                    for is in 1:NS
                        c_coh[ij, ik, it] = c_coh[ij, ik, it] + c[ij, ia, ip, is, ik, it]*phi[ij, ia, ip, is, ik, it]
                        l_coh[ij, ik, it] = l_coh[ij, ik, it] + l[ij, ia, ip, is, ik, it]*phi[ij, ia, ip, is, ik, it]
                        y_coh[ij, ik, it] = y_coh[ij, ik, it] + eff[ij, ik]*theta[ip]*eta[is]*l[ij, ia, ip, is, ik, it]*phi[ij, ia, ip, is, ik, it]
                        a_coh[ij, ik, it] = a_coh[ij, ik, it] + a[ia, ik]*phi[ij, ia, ip, is, ik, it]
                        # exclude households who dies
                        if (ij >= JR && ia == 0 && (kappa[0] <= 1e-10 || kappa[1] <= 1e-10))
                            continue
                        end
                        
                        if (aplus[ij, ia, ip, is, ik, it] < 1e-4) 
                            FLC[ij, ik, it] = FLC[ij, ik, it] + phi[ij, ia, ip, is, ik, it]
                        end

                        VV_coh[ij, ik, it] = VV_coh[ij, ik, it] + VV[ij, ia, ip, is, ik, it]*phi[ij, ia, ip, is, ik, it]
                        m_coh[ij, ik]      = m_coh[ij, ik] + phi[ij, ia, ip, is, ik, it]
                        beq_coh[ij, ik, it] = beq_coh[ij, ik, it] + a[ia, ik]*(1.0 + rn[it])*(1.0 - psi[ij, it])*phi[ij, ia, ip, is, ik, it]/psi[ij, it] #
                    end
                end
            end
        end
...
function government(it)
    ...
    pen[JR:JJ, 1, it] .= kappa[it]*INC[itm]  
    PP[it] = 0.0
    for ij in JR:JJ
        for ik in 1:2

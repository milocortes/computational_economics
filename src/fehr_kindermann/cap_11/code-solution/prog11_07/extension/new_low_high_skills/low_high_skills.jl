...
for param = [:c_coh, :l_coh, :y_coh, :a_coh, :v_coh, :VV_coh, :beq_coh]
    @eval global $param = OffsetArray(zeros(JJ, SS, TT+1), 1:JJ, 1:SS, 0:TT)
end

# bequest variables
global omega = zeros(JJ)
global beq = OffsetArray(zeros(JJ, SS, TT+1) , 1:JJ, 1:SS, 0:TT)
global GAM = OffsetArray(zeros(JJ, SS, TT+1) , 1:JJ, 1:SS, 0:TT)

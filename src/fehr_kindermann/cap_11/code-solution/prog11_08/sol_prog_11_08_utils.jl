...
function foc(x_in)
    ...
    available = (1.0+rn[it_com])*a[ia_com] + penp[ij_com, it_com, ir_com] + v_ind
            ...
            for ip in 1:NP
                for is in 1:NS
                    ...
                    for is_p in 1:NS
                        ...
                        EV[ij, ia, ir, ip, is, it]  = EV[ij, ia, ir, ip, is, it] + pi[is, is_p]*VV[ij, ia, ir, ip, is_p, it]
                    end
                    RHS[ij, ia, ir, ip, is, it] = ((1.0+rn[it])*beta*RHS[ij, ia, ir, ip, is, it])^(-gamma)
...
function LSRA()
            ...
            for ir = 0:NR
                for ip = 1:NP
                    for is = 1:NS
                        ...
                        v_tilde = max(v_tilde, -((1.0+rn[1])*a[ia] + penp[ij, 1, ir] + wn[1]*eff[ij]*theta[ip]*eta[is]*0.99 + v[ij, ia, ir, ip, is, 1]))
    ...
    for it = TT:-1:1
        ...
        if (it == TT)
            ...
            PV_trans = PV_trans*(1.0+n_p)/(1.0+r[it+1]) + v[1, 0, 0, 1, 1, it]
        end
    end

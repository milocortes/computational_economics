...
function foc(x_in)
    ...
    # determine labor
    if (ij_com < JR)
        lab_com = min( max(nu*(1.0-phi_l*Float64(ip_com))  +(1.0-nu)*(a_plus-available)/(wage*(1.0-tauw[it_com]-tau_impl[ij_com,it_com])), 0.0) , 1.0-phi_l*Float64(ip_com)-1e-10)
    else
        lab_com = 0.0
    end
...
function LSRA()
            ...
            for ir = 0:NR
                for ip = 0:NP
                    for is = 1:NS
                        ...
                        v_tilde = max(v_tilde, -((1.0+rn[1])*a[ia] + penp[ij, 1, ir, ip] + wn[1]*eff[ij]*theta[ip]*eta[is]*0.99 + v[ij, ia, ir, ip, is, 1]))
    ...
    for it = TT:-1:1
        for ip = 0:NP
            ...
            if (it == TT)
                ...
                PV_trans = PV_trans*(1.0+n_p)/(1.0+r[it+1]) + v[1, 0, 0, ip, 1, it]
            end
        end
    end
    ...
    for it = TT:-1:1
        for ip = 0:NP
            ...
            v[1, 0, 0, ip, :, it] = v[1, 0, 0, ip, :, it] .+ v_tilde

            # aggregate transfers
            v_coh[1, ip, it] = v[1, 0, 0, ip, 1, it]
